...
class A2C:
    ...
    def policy_loss(self, trajectory):
        ...
        policy improvement
        '''
        return - torch.mean(trajectory['log_probs'] * (trajectory['value_targets'].detach() - trajectory['values'].detach()))

    def value_loss(self, trajectory):
        '''
        policy evaluation
        '''
        return torch.mean(torch.pow(trajectory['value_targets'].detach() - trajectory['values'], 2))
    
    def entropy(self, trajectory):
        '''
        entropy regularization
        '''
        return torch.mean(torch.vstack(list(map(lambda x: x.entropy(), trajectory['distributions']))))
    ...
    def loss(self, trajectory):
        ...
        a2c_loss = policy_loss - self.entropy_coef * entropy + self.value_loss_coef * value_loss
    ...
    def r2(self, trajectory):
        ...
        evaluate values
        '''
        return r2_score(trajectory['value_targets'].detach().numpy(), trajectory['values'].detach().numpy())
    ...
    def step(self, trajectory):
        ...
        grad_norm = torch.nn.utils.clip_grad_norm_(self.policy.model.parameters(), self.max_grad_norm).item()
